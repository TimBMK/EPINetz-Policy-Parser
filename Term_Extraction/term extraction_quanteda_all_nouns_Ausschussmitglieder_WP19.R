...
folder_loc <- "/data/wolfeswenker/Projekte/Policy_Parser/nouns_Ausschussmitglieder_WP19/"
files <- paste0(folder_loc,list.files(folder_loc))[1:2084]
files_modified <- character()

for (i in 1:2084) {
  if(str_contains(files[i], ".RDA")) {
    files_modified <- append(files_modified, files[i])
  }
}

names <- gsub('_neg.RDA', '', files_modified)
names <- gsub('/data/wolfeswenker/Projekte/Policy_Parser/nouns_Ausschussmitglieder_WP19/', '', names)

setwd("/data/wolfeswenker/Projekte/Policy_Parser/Term_Extraction_nouns_Ausschussmitglieder_WP19/")
i=0

while (i <1042) {
  i<-i+1
  j<-i+1
  load(files_modified[j])
  load(files_modified[i])
  ...
  dfm_key <- dfm(corpus_key_t, remove = c(stopwords("german"), stopwords("english"), "amp"),
