...
intercoder_sample <- intercoder_sample %>% # artificial category of wirtschaft + finanzen_haushalt
  mutate(wirtschaft_finanzen_merge = case_when(wirtschaft == TRUE ~ TRUE,
                                               haushalt_finanzen == TRUE ~ TRUE,
                                               .default = FALSE))

intercoder_sample_old <- intercoder_sample_old %>% # artificial category of wirtschaft + finanzen_haushalt
  mutate(wirtschaft_finanzen_merge = case_when(wirtschaft == TRUE ~ TRUE,
                                               haushalt_finanzen == TRUE ~ TRUE,
                                               .default = FALSE))
...
for (field in names(intercoder_sample) %>% .[!. %in% c("_id", "coder", "intercoder_sample")]){
  ...
  alpha_old <- intercoder_sample_old %>% select(`_id`, field, coder) %>% 
    pivot_wider(names_from = `_id`, values_from = field) %>% 
    select(!coder) %>% 
    as.matrix() %>% 
    kripp.alpha("nominal") # T/F data could be considered nominal or ordinal, but results are identical
...
intercoder_eval <- intercoder_sample %>% # get data into expected shape
  ...
  pivot_wider(names_from = coder, values_from = coding, # get into expected data format
              names_glue = "coder_{coder}") %>% 
  left_join(read.xlsx("evaluation_samples/CAP/evaluated/twitter_sample_1.xlsx") %>% 
              select(`_id`, `_source.text`), by = "_id")


## re-export lackluster fields for clarification and reevaluation
twitter_intercoder_sample <- read.xlsx("evaluation_samples/CAP/twitter_sample_intercoder.xlsx")
...
twitter_intercoder_sample %>% filter(`_id` %in% problematic_tweets) %>% 
  ...
  write.xlsx(file = "evaluation_samples/CAP/twitter_recode_sample.xlsx")
