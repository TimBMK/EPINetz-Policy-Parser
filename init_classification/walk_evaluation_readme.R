...
### Concerning the different measures reported in the data:
###   'Score' is the raw Random Walk Score reported by the RWR
###   'ScoreMean' is the Score's mean calculated for duplicated terms - that is, the mean of the scores for walks from different seed terms
###   'ScoreNorm' is the Score normalized FOR EACH RANDOM WALK, that is, for the walk of each seed term, the Score is normalized between 0 and 1
###   'ScoreNormMean' is the ScoreNorm's mean calculated for duplicated terms - that is, the mean of the scores for walks from different seed terms
###   'ScoreNormGroup' is the Score normalized FOR EACH POLICY FIELD, that is, for the walks within each policy field, the Scores are normalized between 0 and 1
###   'ScoreNormGroupMean' is the ScoreNormGroup's mean calculated for duplicated terms - that is, the mean of the scores for walks from different seed terms
### Note that all Normalization and Mean Calculation is done BEFORE any score filtering. Therefore, these scores will differ from any mean calculation applied later
...
walk_terms <- vroom(paste0("init_classification/walk_terms/", rwr_timeframe, ".csv"))
    ...
    # there is also a file with ALL walk terms combined available (init_walk_terms.csv.tar.gz). However, this file is VERY big as the cutoff value was set to 0.5 for evaluation purposes.
    #   In fact, the file is so big, that it will not be feasible to have it loaded in more than one environment without crashes. That is, only one person could work with it at a time.
    #   Therefore, it is adviced to load only single timeframes at a time
...
actual_cutoff = 0.5 # since the initial RWRs had a relatively loose cutoff (ScoreNorm of 0.5) to gain some additional insights into distributions and measures, 
                    #  we can specify a more rigorous cutoff here. Applies to the classification measure specified above. Will ignore seed terms
                    #  Non-reversible, reload walk_terms to restore original data. NULL to skip
                    #  Note that a higher cutoff will favor seed terms, as these are always kept while terms generated by the RWR are dropped
...
cut_frequent_policy_terms = "auto" # Should terms appearing in numerous policy field be cut? 
...
### apply cutoff
if (!is.null(actual_cutoff)) {
  walk_terms <- walk_terms %>% filter(!!as.name(classification_measure) >= actual_cutoff | seed_term == TRUE)
}
...
if (!is.null(cut_frequent_policy_terms)) {
  if (cut_frequent_policy_terms == "auto") {
    classification_terms <- classification_terms %>% 
      filter(lemma %in% (classification_terms %>% 
                           count(lemma, policy_field) %>% 
                           count(lemma) %>% 
    ...
    classification_terms <- classification_terms %>% 
      filter(lemma %in% (classification_terms %>% 
                           count(lemma, policy_field) %>% 
                           count(lemma) %>% 
                           filter(n <= cut_frequent_policy_terms) %>% 
                           pull(lemma)))
  }
}


## A Closer Look at the Actual Walk Terms (after all additional prep)
### the classification_mean_score is the value that will be used to classify the documents. It will always be a mean
### Note, however, that it will be identical with the pre-calculated Mean if one of them was selected as a classification_measure,
### as this also shows the difference between calculating the mean during or after the random walks (that is, after cutoff)
### Any changes in seed term values via the seedterm_value option are also reflected here

classification_terms %>% 
  distinct(lemma, policy_field, !!as.name(classification_measure)) %>% 
  rename(classification_mean_score = !!as.name(classification_measure)) %>%
  left_join(walk_terms %>% distinct(policy_field, NodeNames, seed_term, 
                                    ScoreNormMean, ScoreNormGroupMean),
            by = join_by(policy_field, lemma == NodeNames)) %>% 
  arrange(policy_field, desc(classification_mean_score)) %>% 
  relocate(lemma, policy_field, seed_term, classification_mean_score,
           ScoreNormMean, ScoreNormGroupMean) %>% 
  View()
