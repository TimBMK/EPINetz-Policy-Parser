...
vroom_write(seed_tweets %>% rowwise() %>% mutate(across(.cols = where(is.character),  ~ utf8::as_utf8(.x)), # text as utf8
            ...
            delim = ",", file = "init_classification/no_replies/data_init_seeds_2023-04-10.csv.tar.gz")
...
# saveRDS(seed_tokens, "init_classification/no_replies/tokens_init_seeds_2023-04-10.RDS")
seed_tokens %>% mutate(across(.cols = where(is.character),  ~ utf8::as_utf8(.x))) %>% vroom_write(file = "init_classification/no_replies/tokens_init_seeds_2023-04-10.csv.tar.gz", delim = ",")
