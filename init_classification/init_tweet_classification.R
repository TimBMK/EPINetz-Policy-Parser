...
## read in data
walk_data <- list.files("init_classification/walk_terms") %>% str_remove(".csv") # all available timeframes
  ...
  imap(\(NE, timeframe)
       {
         ...
         res <- classify_documents(
           walk_terms = suppressMessages(vroom(paste0("init_classification/walk_terms/", timeframe, ".csv"))), # data with terms generated by get_rwr_terms()
...
saveRDS(classification_result, file = "init_classification/init_classified_tweets.RDS")
